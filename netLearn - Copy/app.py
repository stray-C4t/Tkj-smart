...
def index():
    # Data Fitur (Gambar 3)
    fitur_list = [
        {"judul": "Quiz", "desc": "Mengerjakan soal mengenai Networking.", "img": url_for('static', filename='photos/foto-nyambung-kabel.JPG')},
        {"judul": "Modul TKJ", "desc": "Materi lengkap dari kelas X hingga XII.", "img": url_for('static', filename='photos/foto-nyambung-kabel.JPG')},
    ...
    guru_all = [
        ...
        {"nama": "Pak Faisal", "mapel": "Komputer Jaringan", "img": "https://i.pravatar.cc/150?u=5"},
        {"nama": "Pak Gama", "mapel": "Komputer Jaringan", "img": "https://i.pravatar.cc/150?u=5"},
        {"nama": "Pak afandi", "mapel": "Komputer Jaringan", "img": "https://i.pravatar.cc/150?u=5"},
        {"nama": "Bu Arini", "mapel": "Komputer Jaringan", "img": "https://i.pravatar.cc/150?u=5"},
        {"nama": "Bu Vera", "mapel": "Komputer Jaringan", "img": "https://i.pravatar.cc/150?u=5"},
        {"nama": "Bu Hafni", "mapel": "Komputer Jaringan", "img": "https://i.pravatar.cc/150?u=5"},
        {"nama": "Bu Tutus", "mapel": "Komputer Jaringan", "img": "https://i.pravatar.cc/150?u=5"},
    ]
...
def dashboard():
    ...
    # 1. Ambil data user
    user = conn.execute('SELECT streak, last_login FROM users WHERE id = ?', (user_id,)).fetchone()
    ...
    conn.execute('UPDATE users SET streak = ?, last_login = ? WHERE id = ?', (current_streak, today, user_id))
    conn.commit()

    # 3. Logika Indikator Hari (Senin=0, Minggu=6)
    # Kita buat list [0,0,0,0,0,0,0], lalu ubah hari ini jadi 1 agar menyala
    day_list = []

    for i in range(6, -1, -1):  # 7 hari terakhir
        ...
        aktivitas = conn.execute('''
			...
			LIMIT 1
		''', (user_id, user_id, user_id, tanggal)).fetchone()
    ...
    # --- Ambil data progress & rekomendasi (kode lama kamu) ---
    progress_data = conn.execute('SELECT up.progress, m.judul, m.deskripsi_singkat, m.id FROM user_progress up JOIN modul m ON up.modul_id = m.id WHERE up.user_id = ? ORDER BY up.id DESC LIMIT 1', (user_id,)).fetchone()
    rekomendasi = conn.execute('SELECT * FROM modul ORDER BY RANDOM() LIMIT 3').fetchall()
    conn.close()
    ...
    return render_template(
        ...
        day_list=day_list # Kirim data hari
    )
...
def account():
    ...
    for i in range(4, -1, -1):
		...
		# VIDEO (kalau belum ada tabel tracking, default 0)
